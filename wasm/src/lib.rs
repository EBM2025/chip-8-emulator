...
impl EmuWasm {
    ...
    #[wasm_bindgen]
    pub fn keypress(&mut self, event: KeyboardEvent, pressed: bool) { // similar to the function in desktop
        let key = event.key();
    ...
    pub fn draw_screen(&mut self, scale: usize) {
        ...
        for i in 0..((WIDTH * HEIGHT) as usize) {
            if display[i] {
                let x_coord = (i % (WIDTH as usize)) as u32;
                let y_coord = (i / (WIDTH as usize)) as u32;
                self.ctx.fill_rect((x_coord * SCALER) as f64, (y_coord * SCALER) as f64, SCALER as f64, SCALER as f64);
            }
        }  
